...
class AppFixtures extends Fixture
{
    ...
    {   
        ...
        for($i=0;$i<count($borneInf);$i++){
            $trarifTransaction=new TarifTransaction();
            $trarifTransaction->setBorneInferieure($borneInf[$i]);
            $trarifTransaction->setBorneSuperieure($borneSup[$i]);
            $trarifTransaction->setValeur($valeurs[$i]);
            $manager->persist($trarifTransaction);
        }
         $manager->flush();
    }
}
