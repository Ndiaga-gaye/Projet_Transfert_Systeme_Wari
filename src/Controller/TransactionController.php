...
class TransactionController extends AbstractController
{
    ...
    public function new(Request  $request, SerializerInterface $serializer, EntityManagerInterface $entityManager, ValidatorInterface $validator): Response
    {
        ...
        $form->submit($Values);
         ...
         //on recupere le numero du compte bancaire
         $parte = $this->getUser()->getCompteBancaire();
         //on recupere le montant qui est dans ce compte
         $repo = $this->getDoctrine()->getRepository(CompteBancaire::class);
         $numcompt = $repo->findOneBy(['numCompt' => $parte]);
         $val = $numcompt->getSolde();
         $repo = $this->getDoctrine()->getRepository(TarifTransaction::class);
         $tar = $repo->findAll();
            ...
            if ($montant < $val) {
                foreach ($tar as $value) {
                    $min = $value->getBorneInferieure();
                    $max = $value->getBorneSuperieure();
